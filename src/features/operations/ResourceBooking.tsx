  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Select a day to view its reservations and manage bookings.
                </p>
              </div>

              {/* Resource Selector Dropdown */}
              <div className="flex items-center gap-2 bg-slate-900/60 border border-slate-800 px-3 py-1.5 rounded-xl">
                <label className="text-[11px] font-bold text-indigo-300 uppercase">Resource:</label>
                <select
            ...
            <div className="flex items-center justify-between bg-slate-950/40 border border-slate-900 rounded-xl px-4 py-3">
              <button
                ...
                className="p-1.5 hover:bg-slate-900 border border-slate-800 rounded-lg text-slate-400 hover:text-white transition cursor-pointer"
              >
            ...
            <div className="grid grid-cols-7 text-center text-xs font-extrabold text-slate-500 uppercase tracking-wider py-2">
            ...
            {/* DAYS GRID CELLS */}
            <div className="grid grid-cols-7 gap-2 max-w-2xl mx-auto w-full">
              {getDaysInMonth(currentMonth).map((day, index) => {
                ...
                return (
                  <button
                    ...
                    className={`relative aspect-square rounded-xl flex flex-col items-center justify-center border text-xs font-bold transition-all cursor-pointer ${
                      isSelected
        ...
        {/* Daily Reservations list (Right Col) */}
        <div className="glass-panel rounded-2xl p-6 flex flex-col h-full min-h-[500px]">
          <div className="mb-4">
            <h2 className="text-xl font-bold text-white">Daily Bookings</h2>
            <p className="text-xs text-slate-400 mt-0.5">
              Active bookings on {new Date(formDate).toLocaleDateString()}
            </p>
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-1">
            {bookings.filter(b => b.status !== 'cancelled' && b.start_time.startsWith(formDate)).length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-slate-500 py-10">
                ...
                <span className="text-xs italic">No reservations booked for this day.</span>
              </div>
            ) : (
              bookings.filter(b => b.status !== 'cancelled' && b.start_time.startsWith(formDate)).map((b) => {
                ...
                return (
                  <div 
                    key={b.id} 
                    className="p-4 rounded-xl border bg-slate-950/45 border-slate-900 hover:border-slate-850 transition-all"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <span className="text-xs font-semibold text-indigo-450 bg-indigo-950/50 px-2 py-0.5 rounded border border-indigo-900/30">
                    ...
                    {isUpcoming && (isOwned || profile?.role === 'admin' || profile?.role === 'asset_manager') && (
                      <button
                        onClick={() => handleCancelBooking(b.id)}
                        className="mt-3 flex items-center gap-1 text-[10px] font-semibold text-rose-450 hover:text-rose-300 hover:bg-rose-955/30 px-2 py-1.5 rounded-lg border border-rose-950 transition-all w-full justify-center cursor-pointer"
                      >
